...
from common import (REPO_ROOT, detect_platform, fmt_ts, load_config,
                    load_secret, secret_search_path)
...
def run_gemini(audio, cfg, lang, provider=None):
    ...
    # Long meetings: split into chunks so a metered/free-tier key spends fewer,
    # bigger requests instead of one call trying to hold an hours-long recording
    # in a single context window. Gemini bills audio at ~32 tok/sec, so even 30
    # min is ~58k tokens -- on a free-tier key the request COUNT is the scarce
    # resource, not the tokens. CHUNK_SEC in the environment overrides
    # config.json's chunk_sec for a one-off long meeting without editing the
    # owner's config.
    duration = audio_duration(audio, ffmpeg)
    chunk_duration = int(os.environ.get("CHUNK_SEC") or cfg.get("chunk_sec", 300))
    if duration > chunk_duration + 30:  # 30s buffer avoids a tiny tail chunk
        num_chunks = int(duration // chunk_duration) + (1 if duration % chunk_duration > 0 else 0)
        chunks = [(i, i * chunk_duration) for i in range(num_chunks)]
        print(f"[transcribe] Audio duration ({duration/60:.1f} min) exceeds limit. "
              f"Split into {len(chunks)} chunks.", flush=True)
    else:
        chunks = [(0, 0)]
        chunk_duration = duration or 0

    # Chunks are cached to disk (chunk_N.txt) so a run interrupted partway through
    # a long meeting resumes instead of re-spending every request already made.
    parts_dir = audio + ".parts"
    os.makedirs(parts_dir, exist_ok=True)

    win_interop = _is_interop(ffmpeg)
    def _wp(p):
        return _winpath(p) if win_interop else p

    def process_chunk(idx, start_time, td):
        ...
        if len(chunks) > 1:
            ...
            subprocess.run([ffmpeg, "-y", "-v", "quiet", "-nostdin", "-ss", str(start_time),
                            "-t", str(chunk_duration), "-i", _wp(audio), "-c", "copy",
                            _wp(chunk_wav)], check=True, timeout=180)
        else:
            chunk_wav = audio
        ...
        if len(chunks) > 1:
            prompt += (f"\nThis is Part {idx + 1} of {len(chunks)} of the meeting. "
                       f"Keep speaker labels consistent if possible.")
        ...
        text, reason, data = None, "", {}
        for temp in (0.1, 0.3, 0.5):
            body = {"contents": [{"parts": [audio_part, {"text": prompt}]}],
                    "generationConfig": {"temperature": temp, "maxOutputTokens": 65536}}
            try:
                data = _gemini_req(f"{base}/v1beta/models/{model}:generateContent", body, key)
            except urllib.error.HTTPError as e:
                raise EngineSkip(f"Gemini HTTP {e.code} on Part {idx + 1}: {e.read().decode()[:300]}")

            cand = (data.get("candidates") or [{}])[0]
            reason = cand.get("finishReason", "")
            if reason == "RECITATION":
                print(f"[transcribe] Recitation block on Part {idx + 1} at temp {temp}. "
                      f"Retrying with a higher temperature...", flush=True)
                continue
            try:
                ...
                if reason and reason != "STOP":
                    print(f"[transcribe] Blocked on Part {idx + 1}: {reason}. "
                          f"Retrying with a higher temperature...", flush=True)
                    continue
                raise EngineSkip(f"Gemini returned no text on Part {idx + 1}: {json.dumps(data)[:300]}")

        if not text:
            raise EngineSkip(f"Gemini transcription failed (safety/recitation) on Part {idx + 1}")
    ...
    with tempfile.TemporaryDirectory() as td:
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(chunks), 15)) as executor:
            futures = {executor.submit(process_chunk, idx, st, td): idx for idx, st in chunks}
            for future in concurrent.futures.as_completed(futures):
                results.append(future.result())  # a real failure aborts the whole batch
